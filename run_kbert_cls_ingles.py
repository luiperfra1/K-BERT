...
class BertClassifier(nn.Module):
    ...
    def forward(self, src, label, mask, pos=None, vm=None):
        ...
        # output = output[:, -1, :] # 只要序列中最后一个token对应的输出，（因为lstm会记录前边token的信息）
        # output = output.reshape(-1)
        # output = torch.transpose(output, 0, 1).contiguous()
        # output = output.view(output.shape[0], -1)

        # TODO 效果差可以再加一层tanh+MLP
        logits = self.fc(out)
...
def main():
    ...
    parser.add_argument("--train_path", type=str, required=False, default="data/implicit_hate_v1_stg1_posts/train.tsv",
                        help="Path of the trainset.")
    parser.add_argument("--dev_path", type=str, required=False, default="data/implicit_hate_v1_stg1_posts/dev.tsv",
                        help="Path of the devset.")
    parser.add_argument("--test_path", type=str, required=False, default="data/implicit_hate_v1_stg1_posts/test.tsv",
    ...
    # kg
    parser.add_argument("--kg_name", required=False, default="brain/kgs/triples.spo", help="KG name or path")
